...
const breadcrumbLastItem = document.querySelector('#breadcrumb-last-item');
...
const renderCategory = (meal) => {
    category.textContent = meal.strCategory;
    category.href = `/category.html?c=${meal.strCategory.toLowerCase()}`;
};
...
const renderInstructions = (meal) => {
    instructions.innerHTML = meal.strInstructions.replace(/\r\n/g, '<br><br>');
};

const renderVideo = (meal) => {
    video.src = meal.strYoutube.replace('watch?v=', 'embed/');
};

const renderBreadcrumbItem = (meal) => {
    const category = meal.strCategory;
    breadcrumbItem.textContent = category;
    breadcrumbItem.href = `/category.html?c=${category.toLowerCase()}`;
    breadcrumbLastItem.textContent = meal.strMeal;
};
